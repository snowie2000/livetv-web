...
export default function Option(props: optProps) {
  ...
  const ConfigMgr = () => <Form labelCol={{ span: 6 }} form={cfgForm}>
    ...
    <Form.Item name="baseurl" label="Baseurl" rules={[{ required: true }]}>
      <Input placeholder={window.location.origin}
        suffix={
  ...
  const PwdMgr = () => <Form labelCol={{ span: 7 }} form={pwdForm}>
  ...
  const About = () => <div>
    <div><a href="https://github.com/zjyl1994/livetv">LiveTV!</a> by <a href="https://github.com/zjyl1994">zjyl1994.</a></div>
    <div>Forked by <a href="https://github.com/snowie2000">snowie2000</a></div>
    <div>Made with <span style={{ color: '#e25555' }}>♥</span> in Kwangtung.</div>
    <div><a href="/log" target='_blank'>View log</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a onClick={() => doLogout.mutate()}>Log out</a></div>
  </div>

  const { data: cfgData } = useQuery("live-config", () =>
    api.get("/getconfig")
  ...
  const doUpdateConfig = useMutation(values => api.post("/updconfig", values, {
    headers: {
  ...
  const doChangePassword = useMutation(values => api.post("/changepwd", values, {
    headers: {
      "Content-Type": "application/x-www-form-urlencoded"
    }
  }))
  ...
  function handleOk() {
    switch (activeTab) {
      case "config": {
        cfgForm.validateFields().then(doUpdateConfig.mutateAsync).then(() => props.onClose())
        break
      }
      case "password": {
  ...
  return (
    <Modal title="Options" open={props.visible} onCancel={props.onClose} onOk={handleOk}>
