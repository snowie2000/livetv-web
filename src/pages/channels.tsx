...
import { CheckCircleFilled, CheckOutlined, CloseCircleFilled, CopyOutlined, DeleteOutlined, EditOutlined, MenuOutlined, QuestionCircleFilled, SettingOutlined, WarningFilled } from '@ant-design/icons';
...
export default function Channels() {
  ...
  const doAddChannel = useMutation((ci: ChannelInfo) =>
  ...
  const doDeleteChannel = useMutation((id: string) =>
    api.get("/delchannel", { params: { id } }), {
    onSuccess() {
      defaultQueryClient.invalidateQueries('channelList')
    },
    onError(error: AxiosResponse) {
  ...
  const doUpdateChannel = useMutation((ci: ChannelInfo) =>
  ...
  const { data: channels, isLoading: chLoading } = useQuery('channelList',
    () => api.get("/channels", {
      responseType: "json"
    }).then(res => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.toolbar}>
        ...
        <div>
          <Button type='primary' onClick={handleAddChannel} style={{ marginLeft: '5px' }}>New channel</Button>
        </div>
      </div>
      <Table size='middle'
        locale={{
